...
pub struct FinderEntry {
    ...
    // Pre-lowercased for zero-allocation scoring
    name_lower: String,
    group_lower: String,
    cwd_lower: String,
}
...
impl FinderState {
    ...
    pub fn open(&mut self, tree: &[TreeNode], show_dead: bool) {
        ...
        self.query.clear();
        self.refilter(); // handles cursor=0, filtered list, and sorting
    }
        ...
        for node in nodes {
            match node {
                ...
                TreeNode::Session(s) => {
                    ...
                    let group_name = parent_group.to_string();
                    let cwd = s
                        .cwd
                        .as_ref()
                    ...
                    out.push(FinderEntry {
                        ...
                        group_lower: group_name.to_lowercase(),
    ...
    fn score(entry: &FinderEntry, query_lower: &str) -> u32 {
        ...
        if entry.name_lower.starts_with(query_lower) {
            ...
            80
        } else if entry.group_lower.starts_with(query_lower) {
            60
        } else if entry.group_lower.contains(query_lower) {
            40
        } else if entry.cwd_lower.contains(query_lower) {
            20
        } else {
            0
        }
...
mod tests {
    ...
    fn test_filter_by_group_name() {
        ...
        let mut state = FinderState::new();
        state.open(&tree, true);
        state.query = "api-work".to_string();
        state.refilter();
        let results = state.results();
    ...
    fn test_filter_by_cwd() {
        ...
        let mut state = FinderState::new();
        state.open(&tree, true);
        state.query = "/Users/dev/Code/website".to_string();
        state.refilter();
        let results = state.results();
    ...
    fn test_no_match_returns_empty() {
        ...
        let mut state = FinderState::new();
        state.open(&tree, true);
        state.query = "zzzznotfound".to_string();
        state.refilter();
    ...
    fn test_scoring_priority() {
        let entry = FinderEntry {
            ...
            name_lower: "auth-refactor".to_string(),
            group_lower: "work".to_string(),
            cwd_lower: "/home/user/code".to_string(),
        };
